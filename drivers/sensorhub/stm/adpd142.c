...
static int adpd_parse_dt(struct adpd142_data *data, struct device *dev)
{
	...
	if (of_property_read_string(this_node, "adpd142,i2c_1p8",
		&data->i2c_1p8) < 0)
...
adpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "%s, is called Success\n", __func__);

	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dev_err(&client->dev, "client not i2c capable\n");
		err = -ENODEV;
		dev_err(&client->dev, "[SENSOR] %s - exit_check_functionality.\n", __func__);
	...
	if (pst_adpd == NULL) {
		err = -ENOMEM;
		dev_err(&client->dev, "[SENSOR] %s - exit_mem_allocate.\n", __func__);
	...
	if (err) {
		dev_err(&client->dev, "%s, parse dt fail\n", __func__);
	...
	if (err < 0) {
		dev_err(&client->dev, "%s adpd142 regulator on fail err = %d\n",
			__func__, err);
		goto err_regulator_enable;
	}
	...
	case ADPD_CHIPID(4):
		...
		dev_info(&client->dev, "chipID value = 0x%x\n", u16_regval);
	break;
	default:
		err = 1;
	break;
	...
	if (err) {
		ADPD142_dbg("chipID value = 0x%x\n", u16_regval);
		dev_err(&client->dev, "chipID value = 0x%x\n", u16_regval);
		dev_err(&client->dev, "[SENSOR] %s - exit_chipid_verification.\n", __func__);
	...
	if (adpd142_initialization(pst_adpd, id)){
		dev_err(&client->dev, "[SENSOR] %s - exit_adpd142_init_exit.\n", __func__);
	...
	err = sensors_register(pst_adpd->dev, pst_adpd, hrm_sensor_attrs,
		"hrm_sensor");
	if (err) {
		dev_err(&client->dev, "[SENSOR] %s - cound not register hrm_sensor(%d).\n",
			__func__, err);
